...
When GEPA writes no optimized prompts (no candidate beats base on dev), the optimized configuration is the base
prompts. It is not run on holdout; the report's PRIME + GEPA section says so, and the GEPA lift is 0 by
construction (task 3.2, user decision).
